...
class TestSaper(unittest.TestCase):
    ...
    def test_two(self):
        ...
        r = pygame.rect.Rect((5, 5), (1, 1))
        # (5, 5) to koordy - oryginalnie get_pos, (1, 1) to wymiary kwadracika ktory reprezentuja kliknięcie.
    ...
    def test_eleven(self):
        """By odpowiednio wykonać ten test musialbym stworzyc nowe okno gry ale w __init__ okna

        gry wystepuje pole z "kotkiem" symbolizującym wygraną lub przegraną
        ktory nie jest zdefiniowany w pliku testów tak samo jak timer.
        Z tego powodu będe testować jedynie wybrane linie kodu, które działają
        tak samo jak w oficjalnym programie.
        """
        n = 5
        m = 5
        topBarHeight = 75
        ...
        sq.Square.flagged_count = 0
        squares = [
            [sq.Square(i * squareHeight - topBarHeight,
                       j * squareWidth, squareHeight, squareWidth, 0, ic.default) for j
             in range(m)] for i in range(n)]
        ...
        '''reset pól, jest, to po prostu przygotowanie od nowa planszy,
        
        w ten sam sposob odbywa się to w programie z zaznaczeniem,
        że w programie generuje się od razu nowa plansza,
        z losowo rozmieszczonymi bombami w innych miejscach 
        w tym tescie nie tworze tablicy fields ukazującej rozmieszczenie bomb.
        '''
        squares = [
            [sq.Square(i * squareHeight - topBarHeight, j * squareWidth, squareHeight, squareWidth, 0, ic.default) for j
             in
    ...
    def test_twelve(self):
        ...
        if y == pygame.K_y and xyzzySequence[0]:
            xyzzySequence[0] = False
            xyzzySequence[1] = True
        if z == pygame.K_z and xyzzySequence[1]:
            xyzzySequence[1] = False
            xyzzySequence[2] = True
        if z == pygame.K_z and xyzzySequence[2]:
            xyzzySequence[2] = False
            xyzzySequence[3] = True
        if y == pygame.K_y and xyzzySequence[3]:
